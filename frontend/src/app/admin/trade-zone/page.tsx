...
/**
 * Admin list page for the Trader Arsenal. Each row mirrors the public
 * card layout (icon + name + description) so the admin sees what
 * they're managing at a glance, plus a compact metadata strip
 * (order, launch destination) and the standard Enable/Edit/Delete
 * actions.
 */
    ...
    <AdminShell>
      <div className="mb-8 flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="font-body text-sm text-wool-muted mt-1 max-w-2xl">
            Manage the tool cards on{' '}
            <code className="text-gold">/trade-zone</code>. Each tool has a
            name, description, icon, an optional launch URL (defaults to{' '}
            <code className="text-gold">/programs</code>), and a button
            label. Reorder via the Order field on each tool — lower
            numbers show first.
          </p>
        </div>
        <div className="flex items-center gap-2 shrink-0">
          ...
          <Link
            ...
            className="font-display text-xs tracking-[0.2em] uppercase text-gold tartan-button px-6 py-2.5 rounded-sm hover:text-gold-light transition-all"
          >
            + Add Tool
          </Link>
        </div>
      ...
      <div className="space-y-3">
        {tools.map((t) => {
          ...
          const launchTo = t.launchUrl?.trim() || '/programs';
          return (
            <div
              key={t.id}
            ...
            >
              {/* Icon block — matches the public card's icon styling */}
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-3 mb-1 flex-wrap">
                  ...
                  {t.launchUrl && (
                    <span className="inline-block font-body text-[9px] tracking-[0.2em] uppercase text-gold bg-gold/10 border border-gold/30 px-2 py-0.5 rounded-sm">
                      Custom URL
                    </span>
                  )}
                </div>
                <p className="font-body text-xs text-wool-muted line-clamp-1 mb-2">
                  {t.description}
                </p>
                <div className="flex flex-wrap items-center gap-x-3 gap-y-1 font-body text-[11px] text-wool-muted/70">
                  ...
                  <span className="truncate max-w-[420px]">
                    {t.launchLabel || 'Launch'} →{' '}
                    <span className="text-gold">{launchTo}</span>
                  </span>
                </div>
              </div>
